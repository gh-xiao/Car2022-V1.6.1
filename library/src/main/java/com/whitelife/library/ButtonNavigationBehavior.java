...
public class ButtonNavigationBehavior extends CoordinatorLayout.Behavior<View> {


    private TransitionSet transitionSet;

    private final static int TRANSLATION_DY =10;

    public ButtonNavigationBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        if(dy> TRANSLATION_DY){
            viewInVisible(child,coordinatorLayout);
        }else if(dy<-TRANSLATION_DY){
